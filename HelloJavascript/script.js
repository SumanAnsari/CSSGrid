...
function getRelativeCoordinates(mouse, element){
  ...
  let coords = {
    ...
    y: mouse.clientY - rect.getScrollPercent
  };
}
...
for(let i = 0; i < sections.length; i++)
{
  ...
  sections[i].addEventListener('mousemove', function(){
    coords = getRelativeCoordinates(mouse, sections[i]);
    console.log("mouse moving inside section!", mouse.clientX, mouse.clientY,
  coords.x, coords.y);
...
var intervalHandle = setInterval(function(){
  ...
  newDiv.style.backgroundColor = 'rgb('+255 * Math.random()+', '+255 * Math.random +')';
